...
public class AbilityRunner : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Composite Pattern
        if(Input.GetKey(KeyCode.F) && Input.GetKey(KeyCode.LeftAlt))
        {
            // Combo
            currentAbility = new SequenceComposite(new IAbility[] {new RageAbility(),new CoolDownDecorator(new FireballAbility())});
        }
    }
}
...
{
    private IAbility[] childrenCombo;

    public SequenceComposite(IAbility[] childrenCombo)
    {
        this.childrenCombo = childrenCombo;
    }

    public void Use(GameObject p_currentObject)
    {
        foreach(var ability in childrenCombo)
        {
            ability.Use(p_currentObject);
        }
    }
}
